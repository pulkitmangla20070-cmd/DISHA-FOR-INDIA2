...
const LeaderboardWidget = ({ topVolunteers, loading, currentRank }) => {
    ...
    return (
      <div className="card">
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '1rem' }}>
          ...
          <h3 style={{ margin: 0, fontSize: '1rem' }}>Top Volunteers</h3>
        </div>
        {[1, 2, 3, 4, 5].map(i => (
  ...
  if (!topVolunteers || topVolunteers.length === 0) {
    return (
      <div className="card" style={{ textAlign: 'center', padding: '2rem 1rem' }}>
        ...
        <p style={{ color: 'var(--color-body)', fontSize: '0.875rem' }}>
  ...
  const getRankIcon = (rank) => {
    ...
    return <span style={{ width: '16px', textAlign: 'center', fontSize: '0.75rem', fontWeight: 600 }}>{rank}</span>;
  };
    ...
    <div className="card">
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '1rem' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
          <Trophy size={18} className="text-primary" />
          <h3 style={{ margin: 0, fontSize: '1rem' }}>Leaderboard</h3>
        </div>
        {currentRank && (
          <div style={{
            fontSize: '0.75rem',
        ...
        {topVolunteers.slice(0, 5).map((vol, index) => {
          ...
          return (
            <div key={vol.userId || vol._id} style={{
              ...
              <div style={{ fontSize: '0.8rem', color: 'var(--color-body)', display: 'flex', alignItems: 'center', gap: '0.25rem' }}>
